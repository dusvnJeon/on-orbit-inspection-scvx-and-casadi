...
def build_and_solve_scvx_subproblem(
    ...
    and fixed trust regions. The nonconvex KOZ outside-ellipsoid constraint is
...
def run_scvx(
    ...
    for iteration in range(1, params.max_iters + 1):
        ...
        X_new = latest["X"]
        U_new = latest["U"]
        metrics = compute_constraint_metrics(params, X_new, U_new, phase_durations)
        ...
        if max_change <= params.change_tol and max_violation <= params.violation_tol:
            break
...
def print_summary_header() -> None:
    print(
        "it | status             | objective    | solve[s] | change   | max viol | "
        "KIZ      KOZ      LOS      DOCK     v        F        term     link"
    )
...
def print_summary_row(row: dict) -> None:
    print(
        ...
        f"{row['solve_time']:8.3f} | {row['max_change']:8.2e} | {row['max_violation']:8.2e} | "
        f"{row['max_kiz_violation']:8.1e} {row['max_koz_violation']:8.1e} "
        f"{row['max_los_violation']:8.1e} {row['max_docking_violation']:8.1e} "
        f"{row['max_velocity_violation']:8.1e} {row['max_force_violation']:8.1e} "
        f"{row['terminal_error']:8.1e} {row['linkage_residual']:8.1e}"
    )


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SCvx/SCP translational CVXPY baseline.")
    ...
    parser.add_argument("--output_csv", type=str, default="rockit_outputs/scvx_translation_solution.csv")
    parser.add_argument("--plot_path", type=str, default="rockit_outputs/scvx_translation_trajectory.png")
...
def main() -> None:
    args = parse_args()
    params = ScvxParams(max_iters=args.max_iters, delta_x=args.delta_x, delta_u=args.delta_u)
